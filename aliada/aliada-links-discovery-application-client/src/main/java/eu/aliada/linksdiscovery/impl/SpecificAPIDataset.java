...
/**
 * Links discovery process which uses specific APIs for discovering links. 
 * 
 * @author Idoia Murua
 * @since 2.0
 */
public class SpecificAPIDataset {
	...
	/** Query to get resources to look for linked entities in VIAF 
	 * (persons' and organisations' names). */
	private final String queryVIAF; 
	/** Query to get resources to look for linked entities in Open Library */
	private final String queryOpenLibr; 
	/** Query to get resources to look for linked entities in LOBID */
	private final String queryLOBIDOrg; 
	/** Query to get resources to look for linked entities in LOBID (bibl. res) */
	private final String queryLOBIDBibRes;
	/** Query to get resources to look for linked entities in Library Of Congress */
	private final String queryLCSubjectHead;
	/** Property name to generate new triples with links to VIAF. */
	private final String propertyNameVIAF;
	/** Property name to generate new triples with links to LOBID. */
	private final String propertyNameLOBID;
	/** Property name to generate new triples with links to Open Library. */
	private final String propertyNameOpenLibr;
	/** Property name to generate new triples with links to Library of Congress Subject Headings. */
	private final String propertyLCSubjectHead;
	 ...
	 * @since 2.0
	 */
	public String[] searchExternalDatasetLOBID (final String text, final String externalDatasetName) {
	 ...
	 * @since 2.0
	 */
	public String[] searchExternalDatasetVIAF (final String text, final String externalDatasetName) {
		String[] searchResults = null;
	 ...
	 * @since 2.0
	 */
	public String[] searchExternalDatasetOpenLibr (final String text, final String externalDatasetName) {
		String[] searchResults = null;
	 ...
	 * @since 2.0
	 */
	public String[] searchExternalDatasetLC (final String text, final String externalDatasetName) {
	 ...
	 * @since 2.0
	 */
	public String[] searchExternalDataset (final String text, final String externalDatasetName) {
		String[] searchResults = null;
	...
	/**
	 * It performs the search in the external datasets and inserts the 
	 * discovered links in the appropriate graph.
	 *
	 ...
	 * @since 2.0
	 */
	public int searchProcess (final JobConfiguration jobConf, final SubjobConfiguration subjobConf) {
		String query = null;
		String propertyName = null;
		int numLinks = 0; 
		String logExtDatasetName = "";
		//Select the appropriate query and property name to use, 
		//depending on the external dataset
		...
		if((query!= null) && (propertyName!= null)) {
			final Integer maxLimit = Integer.MAX_VALUE;
			final int pageSize = MAX_PAGE_SIZE;
			Integer offset = 0;
			Integer limit = Math.min(pageSize, maxLimit - offset);
			// Get some of the URIs of the resources to use to discover 
			//links towards external datasets.
			...
			while (resToLink.length > 0) {
				...
				for (int i=0; i<resToLink.length;i++){
					final String[] externalRes = searchExternalDataset(resToLink[i].getText(), subjobConf.getName());
					if(externalRes != null) {
						for (int j=0; j<externalRes.length;j++) {
							final String triple = "<" + resToLink[i].getResURI() + "> <" + propertyName + "> <" + externalRes[j] + ">";    
				...
				// Get the rest of the URIs of the resources to use to discover
				//links towards external datasets.
